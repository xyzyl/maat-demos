...
use deploy_runner::{build_router, AppState, Config};
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "deploy_runner=info".into()),
        )
        .init();

    let config = Config::from_env()?;
    info!("starting deploy-runner");
    info!("  bind:         {}", config.bind_addr);
    info!("  gateway:      {}", config.maat_gateway_url);
    info!("  environments: {:?}", config.allowed_environments);
    info!("  services:     {:?}", config.allowed_services);
