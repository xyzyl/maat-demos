...
use outreach_agent::{build_router, AgentStore, Config, Runtime};
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "outreach_agent=info".into()),
        )
        .init();

    let config = Config::from_env()?;
    info!("starting outreach-agent on {}", config.bind_addr);
    info!("  gateway: {}", config.gateway_url);
    info!("  mailer:  {}", config.mailer_url);
    info!("  state:   {}", config.agent_db_path);
